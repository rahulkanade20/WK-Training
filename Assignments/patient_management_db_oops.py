...
class Patient:
    ...
    def __init__(self, name, address, mobile, age, status):
...
def insertRecord(conn):
    ...
    newPatient = Patient(name, address, mobile, age, "Active")

    query = f"insert into Patients ('pName', 'pAge', 'pAddress', 'pMobile', 'pStatus') values ('{newPatient.name}', '{newPatient.age}', '{newPatient.address}', '{newPatient.mobile}', '{newPatient.status}')"
